...
def create_LDA_data():
    ...
    merge_tdata=None #holds LDA on training data for 22 chromosomes
    merge_idata=None #holds LDA on independent data for 22 chromosomes
    for chrm in chromosomes:
        ...
        #probes : those probes considered for LDA
        ...
        if idata_X.shape[1]==train_X.shape[1]:
            if idata_X.isnull().values.any():
                ...
                '''
                #open model from file and fit in
                print("Opening model from file..")
                pkl_filename = '/data2/han_lab/sravani/Data/LDA_Data/' + chrm + '_LDA_model.pkl'
                ...
                train_X_trans=train_X_trans.add_prefix(str(chrm+'_'))
    ...
    merge_idata.to_csv('/data2/han_lab/sravani/LDA/IData_combined_LDA_data.csv',index=False)
    merge_tdata.to_csv('/data2/han_lab/sravani/LDA/TCGA_combined_LDA_data.csv',index=False)

    return merge_tdata,merge_idata

def get_LDA_data():
    tcga_lda=pd.read_csv('/data2/han_lab/sravani/LDA/IData_combined_LDA_data.csv', sep=',')
    idata_lda=pd.read_csv('/data2/han_lab/sravani/LDA/TCGA_combined_LDA_data.csv', sep=',')
    return tcga_lda,idata_lda
...
def pipeline_model_selection(X_train,y_train,X_test,y_test):
    ...
    for idx, gs in enumerate(grids):
        ...
        with open('/data2/han_lab/sravani/LDA/pipeline_results.txt', 'a') as f:
            f.write('\n\n\nEstimator: ' + str(grid_dict[idx]) +
                    ...
                    '\nModel filename: /data2/han_lab/sravani/LDA/' + '_'.join(
                grid_dict[idx].split()) + '_model.pkl')
        dump_file = '/data2/han_lab/sravani/LDA/' + '_'.join(grid_dict[idx].split()) + '_model.pkl'
    ...
    dump_file = '/data2/han_lab/sravani/LDA/best_gs_pipeline.pkl'
    joblib.dump(best_gs, dump_file, compress=1)
...
#def strat_here():
tcga_lda,idata_lda=get_LDA_data()

#removing the labels whose count is less than 100 before creating the model
temp=tcga_lda.groupby(['type']).size().reset_index()
filter=temp[temp[0]>100]
filter_data=tcga_lda[tcga_lda['type'].isin(filter['type'])]
...
model=pipeline_model_selection(X_train,y_train,X_test,y_test)

idata_X=idata_lda.drop(columns=['TCGA Label '])
idata_y=idata_lda['TCGA Label ']
...
with open('/data2/han_lab/sravani/LDA/independent_data_classification_report.txt','w') as f:
